...
| Routine | Workflow graph with agents, gates, human-review pauses, councils, handoffs, retries, and terminal states |
...
| Library | Dashboard organization-content area with separate Knowledge and Artifacts tabs |
...
| Artifact | One immutable organization-owned file revision with a stable ID, lineage, digest, and optional catalog path |
| Human Review | Durable routine pause resolved as `approved`, `changes_requested`, or `rejected` |
